...
| **creater (Orchestrator)** | `business-fullstack-creater/.claude/commands/`(scaffold·full-test·growth-start·contribute-back·cleanup-runner) + `scripts/workflow/`(full_test·live_*·lane_runner_map·learn_log) | `business-fullstack-creater/tests/` | 3 (javax lane URL 컨벤션, JDK8 source/target on JDK17 JAVA_HOME, +T-Probe-LaneRunner-Mismatch: scaffold-lane 과 runner-choice 가 `lane_runner_map.py` 에서 conflate) | `cleanup_runner.py` PowerShell subexpression syntax error (Growth-42 메모, deferred); **T-Probe-LaneRunner-Mismatch**: nexacro 스캐폴드 × javax 러너 조합에서 probe 가 `/api/{entity}` REST 호출(실제 endpoint 는 `/uiadapter/{entity}/select_datalist_map.do`) — Growth-48 후보 |
...
| **javax** | `boot-jdk8-javax` (Spring Boot 2.7) | ✅ Growth-32 (sales), ⚠️ Growth-47 (customer fresh scaffold — L1+L2+L4-빌드 PASS, L4-probe FAIL: T-Probe-LaneRunner-Mismatch) | JAVA_HOME=JDK17, target=1.8 (JDK8 부재 시 우회); Growth-46 G-Jackson 픽스 적용 후 fresh-scaffold 종단검증 시도 |
...
| **T-Probe-LaneRunner-Mismatch** (scaffold lane vs runner deployment conflation) | Growth-47 (deferred to Growth-48) | `lane_runner_map.lane_probe_kind/url` 가 단일 `lane` 인자만 받아 wire protocol(REST vs envelope)을 결정 — 하지만 그 lane 은 `full_test.py --lane` 인자(러너 선택)와 동일하게 다뤄짐. nexacro 스캐폴드를 javax runner 위에 올리면(예: scaffold lane=nexacro + runner lane=javax) probe 가 `GET /api/{entity}` 를 호출하지만 실제 endpoint 는 `POST /uiadapter/{entity}/select_datalist_map.do`. 환류 방향(2안): (a) scaffold-lane 을 `scaffold-report.md`/`endpoints.json`(이미 `context_path: /uiadapter` 보유) 에서 읽기, (b) `full_test.py` 에 `--scaffold-lane` override 추가. 결정은 Growth-48 에서. |
