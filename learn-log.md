...
| **creater (Orchestrator)** | `business-fullstack-creater/.claude/commands/`(scaffold·full-test·growth-start·contribute-back·cleanup-runner·**list-domains**) + `scripts/workflow/`(full_test·live_*·lane_runner_map·learn_log·**list_domains**) | `business-fullstack-creater/tests/` | 4 (javax lane URL 컨벤션, JDK8 source/target on JDK17 JAVA_HOME, +T-Probe-LaneRunner-Mismatch RESOLVED Growth-48: scaffold-lane override via `scaffold-report.md` lane line, +T-Probe-CtxPath-Missing RESOLVED Growth-50: REST probe URL 에 `/uiadapter` context-path prefix 누락 — 모든 runner application.yml `server.servlet.context-path: /uiadapter` 통일, nexacro envelope 만 prefix 갖고 REST 는 미포함 → 404) | `cleanup_runner.py` PowerShell subexpression syntax error (Growth-42 메모, deferred) |
