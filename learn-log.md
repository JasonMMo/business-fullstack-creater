...
| **mybatis (Stage 3)** | `andrej-karpathy-rdb-mybatis/templates/<lane>/`, lane(nexacro/vanilla/jakarta/javax) | `andrej-karpathy-rdb-mybatis/tests/` | 6 (explicit-id MERGE, `@Mapper` bean, typed seed sentinel, REST snake_case URL, Map placeholder case, nexacro `_RowType_` 안전추출) | **vanilla lane 검증대 부재** |
